...
void RenderModule::setLamp()
{
	...
	mProgram->setUniformValue(lightPowerID, GLfloat(0.8*lightDistance*lightDistance));
}
...
void VectorFieldModule::setVertexShaderSource()
{
	vertexShaderSource =
	{
		"#version 330 core \n"
...
void VectorFieldModule::setFragmentShaderSource()
{
	fragmentShaderSource =
	{
		"#version 330 core \n"
...
void VectorFieldModule::setData(std::vector<float> vd, std::vector<int> id, std::vector<float> nd, std::vector<float> cd)
{
	vertexData = vd;
	indexData = id;
	normalData = nd;
...
void VectorFieldModule::appData(std::vector<float> vd, std::vector<int> id, std::vector<float> nd, std::vector<float> cd)
{
	vertexData.insert(vertexData.end(), vd.begin(), vd.end());
	indexData.insert(indexData.end(), id.begin(), id.end());
	normalData.insert(normalData.end(), nd.begin(), nd.end());
...
void VectorFieldModule::delData(int begin, int length)
{
	vertexData.erase(vertexData.begin() + begin, vertexData.begin() + begin + length);
	normalData.erase(normalData.begin() + begin, normalData.begin() + begin + length);
	colorData.erase(colorData.begin() + begin, colorData.begin() + begin + length);
...
void VectorFieldModule::genVertexArray()
{
	mVAO = new QOpenGLVertexArrayObject();
	mVAO->create();
	mVAO->bind();
