...
void RKIntegrator::init(double d, double lr, int s, int m)
{
...
void RKIntegrator::_integrateFromCell(Mesh_3_Cell_iterator ci)
{
	...
	do {
		...
		if (success) {
			...
			//for (int i = 0; i < _lines_in_cell[rkv.ci].size(); ++i) {
			//	if (_lines_in_cell[rkv.ci][i] == pl.vertices.end())
			//		//cout << _lines_in_cell[rkv.ci].size() << endl;
			//		cout<<"WTF .. "<<endl;
			//}
		...
		do {
			...
			if (success) {
				...
				if (terminateNearBoundary || terminateOverlap) {
					//cout<< terminateNearBoundary <<" "<< terminateOverlap <<endl;
					//int nn = 0;
					//while (!pl.vertices.empty() && nn < 1) {
						//pl.vertices.pop_front();
						//++nn;
					//}
...
void RKIntegrator::_clean()
{
	...
	do {
		if (iter->vertices.size() < _step_size*0.5) {
			_streamlines.erase(iter);
		}
		else if (_length(*iter) < 4 * _mesh_3.getAveLen()) {
			_streamlines.erase(iter);
		}
		else {
			++iter;
...
void RKIntegrator::_cleanShortLines()
{
	...
	do {	 
		if (_length(*iter) < th * _mesh_3.getAveLen()) {
			_streamlines.erase(iter);
		}
		else {
			++iter;
...
void RKIntegrator::_tuneDensity()
{
	//// fill valid
	//_sl_valid.reserve(_streamlines.size());
	//for (int i = 0; i < _streamlines.size(); ++i) {
	//	_sl_valid.push_back(true);
	//}
	...
	//		_lines_in_cell[iter->ci].insert(i);
	//	}
	//}

	//// start
	//int num = static_cast<int>(std::floor(0.01*_streamlines.size()));
	//for (int i = 0; i < num; ++i) {
	//	Mesh_3_Cell_iterator ci = _selectHighestDensityCell();
	//	int idx = _pickInvalidLine(ci);
	//	_decreaseDensity(idx);
	//}
}
...
bool RKIntegrator::_computeIntersection(Mesh_3_Cell_iterator ci, EigenVector3d pBary, EigenVector3d vBary, double& th, int& i)
{
	...
	//std::cout << "Point Barycentric Coordinate: " << std::endl;
	//std::cout << pBary.x() << " " << pBary.y() << " " << pBary.z() << " " << (1 - pBary.x() - pBary.y() - pBary.z()) << std::endl;
	//std::cout << "Vector Barycentric Coordinate: " << std::endl;
	//std::cout << vBary.x() << " " << vBary.y() << " " << vBary.z() << " " << (1 - vBary.x() - vBary.y() - vBary.z()) << std::endl;
...
bool RKIntegrator::_computeLocation(PolyVertex rkv, Mesh_3_Vector_3 v, PolyVertex& nrkv)
{
	bool terminate = false;
	PolyVertex pv = rkv;
	//PolyVertex nrkv; // to be computed ...
	...
	do {
		...
		//std::cout << rkv.bary.x() << " " << rkv.bary.y() << " " << rkv.bary.z() << std::endl;
		//std::cout << vBary.x() << " " << vBary.y() << " " << vBary.z() << std::endl;
		//std::cout << "ratio: " << th << " " << i << std::endl;
		...
		if (found) {
			if (fabs(th) > 1) {
				...
				//std::cout << inBary.x() << " " << inBary.y() << " " << inBary.z() << std::endl;
				//std::cout << vBary.x() << " " << vBary.y() << " " << vBary.z() << std::endl;
				//baseTransformBarycentricCoordinate(rkv.ci, inBary, i, outBary);
				//std::cout << outBary.x() << " " << outBary.y() << " " << outBary.z() << std::endl;
...
//int RKIntegrator::_pickInvalidLine(Mesh_3_Cell_iterator ci)
//{
	//int idx = *(_lines_in_cell[ci].begin());
	//_lines_in_cell[ci].erase(_lines_in_cell[ci].begin());
...
void RKIntegrator::_decreaseDensity(int idx)
{
	//for (auto iter = _streamlines[idx].vertices.begin(); iter != _streamlines[idx].vertices.end(); ++iter) {
	//	Mesh_3_Cell_iterator ci = iter->ci;
	//	_lines_in_cell[ci].erase(idx);
	//}
}
